...
def submit_contact():
    ...
    try:
        ...
        contact_id_counter += 1
        return jsonify({'success': True, 'message': 'Sent!', 'id': contact['id']}), 200
...
def get_stats():
    ...
    return jsonify({'total': total, 'new': new, 'read': total - new, 'theft': theft}), 200

@app.route('/api/status', methods=['GET'])
def get_status():
    return jsonify({'status': 'online', 'total_contacts': len(contacts_db), 'timestamp': datetime.now().isoformat()}), 200
